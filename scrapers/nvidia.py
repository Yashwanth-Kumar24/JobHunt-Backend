...
REJECT_TITLE = re.compile(
    r"\b("
    r"principal|staff|director|manager|head|lead|vp|vice\s*president|silicon|circuit|timing|simulation|quantum|"
    r"senior\s+manager|advisor|senior\s+director|distinguished|architect|hardware|cad|power|cosmos|fleet|soc"
    r")\b",
    re.IGNORECASE,
)
...
def scrape(max_pages: int = 15, page_size: int = 20) -> List[Dict]:
    ...
    for page in range(max_pages):
        payload = {
            "appliedFacets": {
                "jobFamilyGroup": ["0c40f6bd1d8f10ae43ffaefd46dc7e78","0c40f6bd1d8f10ae43ffbd1459047e84"],
        ...
        try:
            ...
            print(f"Nvidia page {page + 1}: request failed - {e}")
            break
        ...
        print(f"Nvidia page {page + 1}: scanned={len(postings)} kept={kept}")

        offset += page_size
        time.sleep(0.4)

    deduped = {j["external_job_id"]: j for j in jobs}
    print("Nvidia jobs:", len(deduped))
    return list(deduped.values())


if __name__ == "__main__":
    res = scrape(max_pages=5)
    print("Nvidia jobs:", len(res))
    if res:
        print("Sample:", res[0])
